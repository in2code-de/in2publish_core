...
class ConfigurationMigrationTest implements TestCaseInterface
{
    ...
    {
        $migrationMessages = $this->configContainer->getMigrationMessages();
        if (empty($migrationMessages)) {
            return new TestResult('configuration.migration.okay');
        }
        return new TestResult('configuration.migration.messages', TestResult::WARNING, $migrationMessages);
    }
