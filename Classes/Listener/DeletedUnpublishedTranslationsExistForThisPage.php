    ...
    private function checkSoftDeletedTranslations(
        ...
        $deletedTranslationsLocal = $localQuery->select('uid')
            ...
            ->where(
                ...
                $localQuery->expr()->eq(
                    'l10n_parent',
                    $localQuery->createNamedParameter($languageParentId, \PDO::PARAM_INT)
                ),
                $localQuery->expr()->eq(
                    'sys_language_uid',
        ...
        $foreignTranslations = $foreignQuery->select('uid')
            ...
            ->where(
                $foreignQuery->expr()->eq(
                    'l10n_parent',
                    $foreignQuery->createNamedParameter($languageParentId, \PDO::PARAM_INT)
                ),
                $foreignQuery->expr()->eq(
                    'sys_language_uid',
                    $foreignQuery->createNamedParameter($recordLanguage, \PDO::PARAM_INT)
                ),
